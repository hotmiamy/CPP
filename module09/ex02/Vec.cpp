...
void Vec::getSequence(std::string input)
{
...
void Vec::createPairs(void)
{
	...
	while (size != 0)
	{
		if (this->vec[inx] < this->vec[inx + 1])
			std::swap(this->vec.at(inx), this->vec.at(inx + 1));
		this->Pairs.push_back(std::make_pair(this->vec.at(inx), this->vec.at(inx + 1)));
		inx += 2;
		size--;
	}
}

void Vec::sortPairs(void)
{
...
void Vec::mergeSort(int left, int right) {
    if (left < right) {
...
void Vec::merge(int left, int middle, int right) {
    int n1 = middle - left + 1;
    int n2 = right - middle;

    std::vector<std::pair<int, int> > leftTemp(n1);
    std::vector<std::pair<int, int> > rightTemp(n2);
...
int Vec::binarySearch(std::vector<int> array, int target, int begin, int end)
{
	int mid;
...
void Vec::JacobInsertionSequence()
{
	size_t size;
